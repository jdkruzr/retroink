...
See [Obsidian Clipping Sync](./obsidian-sync.md) for what this pushes and
where. There is a single configured destination (not a list, unlike OPDS/Wi-Fi).

### `GET /api/obsidian`

Returns the current sync configuration. The API key is never returned, only
whether one is set.

```bash
curl http://crosspoint.local/api/obsidian
```
...
```json
{
  "enabled": true,
  "mode": "local_rest_api",
  "baseUrl": "https://192.168.1.50:27124",
  "fallbackBaseUrl": "",
  "notePathTemplate": "Clippings/{book}.md",
  "insecureTls": true,
  "autoSyncOnFileTransfer": false,
  "hasApiKey": true,
  "pendingCount": 3
}
```

`mode` is `local_rest_api` or `webhook`.

### `POST /api/obsidian`

Updates the sync configuration from a JSON object. Any field can be omitted
to leave it unchanged; `apiKey` is only overwritten when present in the body
(the web UI omits it once set, same convention as the OPDS/Wi-Fi password
fields).

```bash
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"enabled":true,"mode":"local_rest_api","baseUrl":"https://192.168.1.50:27124","apiKey":"secret","notePathTemplate":"Clippings/{book}.md"}' \
  http://crosspoint.local/api/obsidian
```

### `POST /api/obsidian/sync`

Immediately drains the pending clipping queue to the configured destination,
stopping at the first failure. Safe to call whenever the web server is
running. Wi-Fi is already up at that point, so this is also what automatic
sync-on-File-Transfer triggers internally.

```bash
curl -X POST http://crosspoint.local/api/obsidian/sync
```

Response:

```json
{"ok":true,"synced":3,"remaining":0,"message":"Success"}
```
