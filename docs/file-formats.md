...
## RetroInk Library (version 1)

`/.retroink/library.bin` begins with a 12-byte header: `RILB` magic (`u32`),
version (`u16`, currently 1), reserved (`u16`), and book count (`u32`). It then
stores one fixed 872-byte record per book: a 512-byte path, 224-byte title,
112-byte author, 64-bit file size, 64-bit sampled fingerprint, 32-bit recent
rank, and one byte of activity/finished/favorite flags (plus alignment padding).
Paths and text are null-terminated UTF-8. The fingerprint hashes file size and
the first and last 4 KiB, so a match is only a recovery suggestion and always
requires review. `library.path.idx`, `library.title.idx`, and `library.view.idx`
contain 32-bit record numbers sorted for path lookup, the saved All Books /
Title A-Z default, and the currently selected Library view. The saved title
index lets later entries open without a folder scan. Reader exits update only
the matching record's shelf flags. `library.dirty` marks a changed book from
file transfer and triggers one reconciliation scan on the next entry.
`library.scan` and `library.cursor` hold a paused scan until it resumes;
`library.orphans.bin` uses the same record layout and a `RIOR` header for old
path candidates. CrossInk caches remain in their original formats.

`/.retroink/move.journal` starts with `RIMV` magic (`u32`), a flags byte,
two path lengths (`u16` each), then the source and destination path bytes.
Flags mark a folder move, recents retention, recovery, and completion of the
backup stage. A journal remaining after reboot is retried before Home opens.
