"""Build the RetroInk web portal: compose pages from shared templates/assets,
...
Source of truth lives under web/:
  ...
  web/assets/chicago.ttf   - UI chrome font, served once at /chicago.ttf.
                             Pre-subset to basic Latin (U+0020-007E) from
                             assets/system6/ChicagoFLF.ttf - the same font
                             the on-device System6 theme uses for its own UI
                             chrome (see System6Fonts.cpp). Regenerate with:
                             .venv/bin/python -m fontTools.subset \
                               assets/system6/ChicagoFLF.ttf \
                               --output-file=web/assets/chicago.ttf \
                               --unicodes="U+0020-007E" --no-hinting \
...
PAGES = {
    "home":     ("HomePageHtml",     "RetroInk",                   "home",     ""),
    "files":    ("FilesPageHtml",    "Files - RetroInk",           "files",    '  <script src="/js/jszip.min.js"></script>'),
    "library":  ("LibraryPageHtml",  "Library - RetroInk",         "library",  ""),
    "settings": ("SettingsPageHtml", "Settings - RetroInk",        "settings", ""),
    "fonts":    ("FontsPageHtml",    "Fonts - RetroInk",           "fonts",    ""),
}
...
def emit_gzip_bytes(path, ident, data):
    gz = gzip.compress(data, compresslevel=9)
    emit_header(path, ident, gz, original_len=len(data))
...
for slug, (ident, title, active, head_extra) in PAGES.items():
    ...
    values = {
        ...
        "cls_home": "", "cls_files": "", "cls_library": "", "cls_settings": "", "cls_fonts": "",
    }
...
# UI chrome font (uncompressed TTF outline data: gzips well, like HTML/CSS).
chicago_ttf = open(os.path.join(WEB, "assets", "chicago.ttf"), "rb").read()
orig, comp = emit_gzip_bytes(os.path.join(OUT, "ChicagoFlfTtf.generated.h"), "ChicagoFlfTtf", chicago_ttf)
print(f"{'ChicagoFlfTtf':18} {orig:>7}B -> {comp:>6}B gz")
