...
def inject_version(env):
    ...
    if pioenv == 'default':
        if os.environ.get('CROSSINK_RC_HASH'):
            ...
            print(f'RetroInk production build version: {version_string}')
        else:
            # A personal fork's own hardware build: just the plain version
            # (e.g. "0.2.0"), not a CI-style "-dev+<branch>" suffix - there's
            # no fleet of parallel dev branches whose flashes need telling
            # apart here, and the branch name isn't something to show on the
            # device's own About/Status screen.
            version_string = get_crossink_version(project_dir)
            print(f'RetroInk build version: {version_string}')
