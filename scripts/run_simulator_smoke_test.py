...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="crossink-sim-smoke-") as temp_dir_name:
        ...
        if args.ui_scale:
            env["CROSSINK_SIMULATOR_SMOKE_UI_SCALE"] = "0" if args.ui_scale == "small" else "1"
        if args.show_clock:
            env["CROSSINK_SIMULATOR_SMOKE_SHOW_CLOCK"] = "1"
        if args.headless:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--env", choices=("simulator", "simulator-X3", "sticky-simulator", "x4-pro-simulator"), default="simulator",
    ...
    parser.add_argument("--ui-scale", choices=("small", "large"), help="UI scale to use during the smoke test")
    parser.add_argument("--show-clock", action="store_true", help="Show the clock in UI and reader headers")
