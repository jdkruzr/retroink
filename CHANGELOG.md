...
- RetroInk can hide the Files shortcut from Home, and now does so by default so Library is the primary book browser. The shortcut can be restored under System > Files & Cache or through web settings.
- RetroInk Home has a separate Library with All Books, To Read, Reading, Finished, and Favorites shelves, four sort choices, A-Z letter jump, favorite pins, and Move to Folder. Browse Files remains available for physical folders.
- Library scans nested collections one book at a time, reads EPUB title and author from a streamed OPF when no metadata cache exists, and stores catalog and sort indexes on the SD card. Back saves the partial scan so it resumes on the next visit.
- Library recognizes a later external move by a sampled file fingerprint and offers a review of old and current progress before recovery. Older orphan bookmark and clipping headers are also considered. Restoring the old record backs up current path-keyed state.

### Changed

- RetroInk Home now keeps Files, Library, Reading Stats, Focus Session, Clippings, File Transfer, and Settings in that order. The Recent Books menu entry is removed; the home book panel still supports continuing a book.
- Automatic finished-book moves, on-device Library moves, web rename/move, and WebDAV MOVE use a shared journaled path that migrates reading caches, bookmarks, clippings, recents, and resume location before reporting success. Folder moves migrate books one at a time.

### Fixed

- The current book title on RetroInk Home now wraps across the available card space instead of being clipped to one line.
- RetroInk's boot eye animation now uses fast refreshes throughout; the X3 startup resync no longer promotes its early frames to full-screen flashes. Sleep-screen full refresh remains available.
- Reopening a finished Library now reads its saved All Books/Title A-Z index immediately instead of rescanning every folder. Reader exits update one book's shelf flags, book transfers trigger one reconciliation scan, and Actions includes Refresh Library for external file changes.
- Library scans and move journals accept the existing `/.retroink` directory on hardware; SdFat reports `mkdir` failure when that directory already exists.
- Renaming or moving a book through the web interface or WebDAV no longer clears its reading record. Destination state conflicts are rejected rather than replacing another book's cache.
- An interrupted move resumes its state migration after the reading state and recents stores load at boot.

## [RetroInk 0.1.0 preview] - 2026-09-13

### Added

- RetroInk readers can optionally show a top-right daily-goal minute countdown. It combines the current reading session with saved daily minutes, refreshes only the small badge when a minute changes, and flashes that badge once when the goal is met. The switch is in Reading Stats settings and the web settings API.
- RetroInk sleep settings add three lightweight Macintosh dialogs: Error 404, Insert Bookmark, and System Nap.
- Focus sessions can run up to 24 hours; the duration editor uses five-minute front-key steps and one-hour side-key steps. A one-second refresh choice redraws while awake and warns that it uses more battery.
- Focus Session is available from RetroInk Home through a Focus Desk with Start and Settings; Reading Stats has an Actions menu for starting Focus or changing the daily goal.
- Focus sessions save a small return-and-countdown record for restoration after a restart.
- Reading Desk adds a daily minutes goal, one-year monochrome reading calendar, current-book status page, and a pixel-block Focus Session timer with selectable refresh cadence.
- Selectable RetroInk theme with striped window headers, monochrome document tiles, square controls, and inverted selections. The window bar incorporates the configured clock and battery indicators, while checkerboard desktop texture surrounds the bottom controls. Existing themes and saved theme values remain available.
- RetroInk menu footers now reserve room above the button strip, and button labels clip within their frames instead of running under the X3 bezel.
- The active Settings category remains visibly inverted while paging through its menu.
- RetroInk now enforces the Large UI scale for readable controls and labels, keeps header titles clear of the clock and battery fields, and renders available book covers with a memory-bounded one-bit dither.
- RetroInk's physical-button hints now float as shadowed keycaps over the checkerboard desktop, and cold boots use a matching RetroInk Macintosh startup window.
- RetroInk is now identified as an AltFlow project built on CrossInk and is the dedicated firmware UI. Its boot Macintosh glances left and right, while the default sleep screen says RetroInk is resting between chapters. Alerts, confirmations, checkbox labels, toggle geometry, file-type icons, empty collections, and progress bars receive matching monochrome Macintosh styling.

### Fixed

- Reading Year now orders days left to right like a book, starting with January 1 at the top-left and leaving only the unused end-of-year squares at the bottom-right; its header names the calendar year.
- Reading Year marks any reading beyond the daily goal as Extra, rather than waiting until twice the goal; Today uses the exact recorded time for Goal met and Goal exceeded.
- Sleep screens hide the clock in RetroInk headers and preserved reader pages; the normal home and reader clocks return after sleep rendering.
- Reading Year now shows exactly the current calendar year's 365 days, or 366 in leap years, without including days from the previous year.
- Focus Session now has its own RetroInk hourglass icon on Home, distinct from the Reading Stats chart.
- Focus Back and End remain responsive on the X3/X4 analog-button ladder throughout a session; Reading Year is one uninterrupted, full-height grid of square day cells.
- RetroInk Focus Desk and Reading Stats now handle Select and Back on the first release, and the timer's displayed End key dismisses the session. Button-release suppression no longer swallows the next tap after a button is already up.
- RetroInk draws the same checker texture behind the header and bottom keys, including unlabeled keycaps; Reading Year fills the portrait page with larger day cells and a bottom legend. Book Status wraps longer book titles.
- Focus and reading-goal editors show one value and use Back and Done hints for their distinct actions. The countdown no longer shows the unrelated refresh-setting label, and preview firmware identifies itself as RetroInk without an unknown Git suffix.
- Reading Stats is visible before the first session, opens on Today from Home, always reaches Reading Year, and gives every RetroInk footer button a matching action. The year grid and Focus timer fit the full screen without crossing the button surround.
- The RetroInk file browser uses a readable path label and no longer draws the tiny hidden-files hint across its footer.

### Changed

- RetroInk's black-and-white sleep screens use a full-panel refresh to clear checkerboard ghosting. Grayscale sleep screens clear the panel fully before their calibrated gray pass.
- Focus displays seconds only with the one-second redraw option. Minute and five-minute modes show remaining minutes (or hours and minutes for long sessions) with a unit label. Manual refresh is removed from the device and web settings, and previously saved Manual values migrate to one-minute redraws.
- Focus stays awake at the device's low idle CPU frequency between scheduled redraws so the front keys can be polled. Wi-Fi stays off during sessions; this uses more battery than deep sleep.
- Focus refresh choices are shown as a selectable four-row list. Countdown tiles now represent each session minute and clear as time passes; the completion dialog flashes before the timer's full refresh.
- The RetroInk sleep card shows only “RetroInk is resting between chapters”; startup keeps the Mac logo and one “Built on CrossInk” line.
