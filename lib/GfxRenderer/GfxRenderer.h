...
class GfxRenderer {
 ...
 private:
  ...
  // Optional app font policy; one pointer, no per-call allocation. It must
  // return a registered font with compatible line metrics and never recurse.
  int (*fontResolver_)(int, const char*, EpdFontFamily::Style) = nullptr;
 ...
 public:
  ...
  void setFontResolver(int (*resolver)(int, const char*, EpdFontFamily::Style)) { fontResolver_ = resolver; }
  ...
  void drawBitmap(const Bitmap& bitmap, int x, int y, int maxWidth, int maxHeight, float cropX = 0,
                  float cropY = 0) const;
  // Draw a 2-bit BMP into the one-bit framebuffer with a fixed 2x2 Bayer
  // pattern. Uses the shared row scratch only; no image-sized buffer.
  void drawBitmapDithered(const Bitmap& bitmap, int x, int y, int maxWidth, int maxHeight) const;
