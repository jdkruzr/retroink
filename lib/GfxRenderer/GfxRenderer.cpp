...
int GfxRenderer::resolveTextFontId(int fontId, const char* text, const EpdFontFamily::Style style) const {
  if (fontResolver_) fontId = fontResolver_(fontId, text, style);
...
void GfxRenderer::drawBitmapDithered(const Bitmap& bitmap, const int x, const int y, const int maxWidth,
  ...
  for (int bmpY = 0; bmpY < bitmap.getHeight(); ++bmpY) {
    if (bitmap.readNextRow(outputRow, rowBytes) != BmpReaderError::Ok) {
      LOG_ERR("GFX", "Failed to read dithered bitmap row %d", bmpY);
      return;
    }
    const int sourceY = bitmap.isTopDown() ? bmpY : bitmap.getHeight() - 1 - bmpY;
    const int screenY = y + (isScaled ? static_cast<int>(std::floor(sourceY * scale)) : sourceY);
    ...
    for (int bmpX = 0; bmpX < bitmap.getWidth(); ++bmpX) {
      ...
      const uint8_t value = (outputRow[bmpX / 4] >> (6 - ((bmpX * 2) % 8))) & 0x3;
      const bool bayerPixel = ((screenX & 1) == 0) && ((screenY & 1) == 0);
      if (value == 0 || (value == 1 && !bayerPixel) || (value == 2 && bayerPixel)) {
        drawPixel(screenX, screenY);
      }
    }
  }
