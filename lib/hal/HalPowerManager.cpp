...
bool HalPowerManager::armTimerWakeup(const uint64_t wakeMicros) const {
  const esp_err_t err = esp_sleep_enable_timer_wakeup(wakeMicros);
  if (err != ESP_OK) {
    LOG_ERR("PWR", "Could not arm focus timer wake (%d)", static_cast<int>(err));
    return false;
  }
  return true;
}

void HalPowerManager::disableWiFiForFocusSession() const { disableWiFiBeforeDeepSleep(); }
