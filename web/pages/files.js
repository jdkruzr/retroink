...
if (currentPath !== "/") {
  ...
  if (leaf) document.title = leaf + " - Files - RetroInk";
}
  ...
  if (files.length === 0) {
    ...
    sortedFiles.forEach((file) => {
      if (file.isDirectory) {
        ...
        fileTableContent += `<td class="actions-col"><div class="action-icon-group"><button class="delete-btn file-action-btn" data-action="delete" data-name="${escapeHtml(file.name)}" data-path="${encodeURIComponent(folderPath)}" data-is-folder="true" title="Delete folder">&times;</button></div></td>`;
        fileTableContent += "</tr>";
      } else {
        ...
        fileTableContent += `<button class="move-btn file-action-btn" data-action="move" data-name="${escapeHtml(file.name)}" data-path="${encodeURIComponent(filePath)}" title="Move file">Move</button>`;
        fileTableContent += `<button class="rename-btn file-action-btn" data-action="rename" data-name="${escapeHtml(file.name)}" data-path="${encodeURIComponent(filePath)}" title="Rename file">Rename</button>`;
        fileTableContent += `<button class="delete-btn file-action-btn" data-action="delete" data-name="${escapeHtml(file.name)}" data-path="${encodeURIComponent(filePath)}" data-is-folder="false" title="Delete file">&times;</button>`;
...
function openUploadModal() {
  ...
  document.getElementById("uploadPathDisplay").textContent = currentPath === "/" ? "/ (Home)" : currentPath;
...
function renderImageGrid() {
  ...
  epubImagesCache.forEach((img) => {
    ...
    if (isCover) {
      ...
      item.innerHTML = `
          <span class="image-state-badge cover-badge">Locked</span>
...
function openFolderModal() {
  document.getElementById("folderPathDisplay").textContent = currentPath === "/" ? "/ (Home)" : currentPath;
...
function exportLogToFile(filename = null, isBatch = false) {
  ...
  let logText = `RetroInk ${crosspointVersion} - EPUB Conversion Log\n`;
