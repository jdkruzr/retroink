...
const char* CrossPointSettings::getDefaultDeviceName() {
  ...
  if (gpio.deviceIsX3()) return "RetroInk X3";
  if (gpio.deviceIsX4()) return "RetroInk X4";
  return "RetroInk";
}
...
bool CrossPointSettings::fromJson(JsonVariantConst doc) {
  ...
  for (const auto& info : getBaseSettingsList()) {
    ...
    if (strcmp(info.key, "focusTimerRefresh") == 0 && value == FOCUS_TIMER_MANUAL) {
      value = FOCUS_TIMER_EVERY_MINUTE;
      needsResave = true;
    }
...
bool CrossPointSettings::verifySleepScreenMigrationContract() {
  ...
  return sleepScreenStorageToMode(minimalSleepStorageValue) == MINIMAL_SLEEP &&
         ...
         sleepScreenStorageToMode(12) == RETROINK_ERROR_404_SLEEP &&
         sleepScreenStorageToMode(13) == RETROINK_INSERT_BOOKMARK_SLEEP &&
         sleepScreenStorageToMode(14) == RETROINK_SYSTEM_NAP_SLEEP &&
