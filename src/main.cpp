...
RTC_NOINIT_ATTR uint32_t focusSleepWakeMagic;
...
constexpr uint32_t FOCUS_SLEEP_WAKE_MAGIC = 0x464F4353;
...
void enterFocusDeepSleep(const uint32_t seconds) {
#ifndef SIMULATOR
  if (seconds == 0 || !powerManager.armTimerWakeup(static_cast<uint64_t>(seconds) * 1000000ULL)) return;
  HalPowerManager::Lock powerLock;
  focusSleepWakeMagic = FOCUS_SLEEP_WAKE_MAGIC;
  deepSleepInProgress = true;
  putTiltSensorToSleepForDeepSleep();
  display.deepSleep();
  mirrorWakeShortPressToNvs();
  LOG_INF("FOCUS", "Sleeping until next timer redraw in %lu seconds", static_cast<unsigned long>(seconds));
  powerManager.startDeepSleep(gpio);
#else
  (void)seconds;
#endif
...
void setupDisplayAndFonts(const bool seamless = false, const bool loadReaderResources = true,
  ...
  display.begin();
#else
  display.begin(seamless, fastSplash);
#endif
  renderer.begin();
...
void setup() {
  ...
  const bool focusWakeToken = focusSleepWakeMagic == FOCUS_SLEEP_WAKE_MAGIC;
  focusSleepWakeMagic = 0;
  ...
  switch (wakeupReason) {
    case HalGPIO::WakeupReason::PowerButton: {
      if (focusWakeToken) {
        LOG_INF("FOCUS", "Power wake during a focus session");
        break;
      }
  ...
  FocusSessionActivity::Snapshot focusSnapshot;
  const bool restoreFocus = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 &&
                            FocusSessionActivity::readSnapshot(rawWakeupCause == ESP_SLEEP_WAKEUP_TIMER, focusSnapshot);
  ...
  // Recents must be loaded before the migration changes their paths.
  if (Storage.exists("/.retroink/move.journal")) {
    if (isNetworkResume) RECENT_BOOKS.loadFromFile();
    BookMoveUtils::recoverPendingMove();
  }
  ...
  const BootResume resume = restoreFocus                ? BootResume::Silent
  ...
  setupDisplayAndFonts(resume != BootResume::Splash, resume != BootResume::Network,
                       resume == BootResume::Splash && SETTINGS.uiTheme == CrossPointSettings::SYSTEM6);
  ...
  if (recoveryFirmwareMode) {
    ...
    LOG_INF("FOCUS", "Restoring focus session: %lu seconds remaining",
            static_cast<unsigned long>(focusSnapshot.remainingSeconds));
    activityManager.replaceActivity(
        std::make_unique<FocusSessionActivity>(renderer, mappedInputManager, std::move(focusSnapshot)));
...
void loop() {
  ...
  if (buttonActivity
    ...
    // X3/X4 front buttons share ADC ladders and cannot wake from deep sleep.
    // Keep Focus polling at the 10 MHz idle clock between sparse redraws.
    powerManager.setPowerSaving(focusActive && !buttonActivity);
  }
  ...
  } else if (!activityManager.hasActivityNamed("FocusSession") && millis() >= allowSleepAt &&
...
#ifndef SIMULATOR
  const uint32_t focusSleep = activityManager.focusSleepSeconds();
  if (focusSleep) {
    enterFocusDeepSleep(focusSleep);
    return;
  }
#endif
  ...
  if (activityManager.skipLoopDelay()) {
    ...
    if (activityManager.isCurrentActivityNamed("FocusSession")) {
      ...
      delay(50);  // Poll the ADC keys without redrawing or busy-spinning.
      return;
    }
