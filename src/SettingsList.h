...
inline const std::vector<SettingInfo>& getBaseSettingsList() {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    add(SettingInfo::Toggle(StrId::STR_AUTO_BACKUP_STATS, &CrossPointSettings::autoBackupStats, "autoBackupStats",
                            StrId::STR_CAT_SYSTEM));
    add(SettingInfo::Value(StrId::STR_READING_GOAL, &CrossPointSettings::readingGoalMinutes, {5, 180, 5},
                           "readingGoalMinutes", StrId::STR_CAT_SYSTEM));
    add(SettingInfo::Toggle(StrId::STR_READING_GOAL_COUNTDOWN, &CrossPointSettings::showReadingGoalCountdown,
    ...
    add(SettingInfo::Value(StrId::STR_FOCUS_MINUTES, &CrossPointSettings::focusSessionMinutes, {0, 60, 5},
                           "focusSessionMinutes", StrId::STR_CAT_SYSTEM));
    add(SettingInfo::Enum(StrId::STR_FOCUS_TIMER_REFRESH, &CrossPointSettings::focusTimerRefresh,
                          {StrId::STR_FOCUS_REFRESH_SECOND, StrId::STR_FOCUS_REFRESH_MINUTE,
                           StrId::STR_FOCUS_REFRESH_FIVE_MINUTES},
                          "focusTimerRefresh", StrId::STR_CAT_SYSTEM)
            .withEnumRawValues({CrossPointSettings::FOCUS_TIMER_EVERY_SECOND,
    ...
    // Persisted and web-exposed, but category-less: it's set from the Library
    // screen's own Actions menu ("Set as Default Shelf"), not the Settings screen.
    add(SettingInfo::String(StrId::STR_LIBRARY_DEFAULT_SHELF, SETTINGS.libraryDefaultShelf,
                            sizeof(SETTINGS.libraryDefaultShelf), "libraryDefaultShelf"));
