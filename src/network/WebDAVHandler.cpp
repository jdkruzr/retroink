...
void WebDAVHandler::handlePut(WebServer& s) {
  ...
  RetroInkLibraryCatalog::noteChangedBook(path.c_str());
...
void WebDAVHandler::handleMove(WebServer& s) {
  ...
  // A MOVE must never replace another book's progress or cache, even when a
  // WebDAV client sends Overwrite: T.
  if (dstExists) {
    s.send(409, "text/plain", "Destination already exists");
    return;
  }
  ...
  file.close();
  bool success = BookMoveUtils::moveWithState(srcPath.c_str(), dstPath.c_str());
...
void WebDAVHandler::handleCopy(WebServer& s) {
  ...
  if (copyOk) {
    RetroInkLibraryCatalog::noteChangedBook(dstPath.c_str());
    s.send(dstExists ? 204 : 201);
  } else {
    Storage.remove(dstPath.c_str());
