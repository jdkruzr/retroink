...
#include "html/ChicagoFlfTtf.generated.h"
...
void CrossPointWebServer::begin() {
  ...
  server->on("/chicago.ttf", HTTP_GET, [this] { handleChicagoFont(); });
  ...
  server->on("/api/library/books", HTTP_GET, [this] { handleGetLibraryBooks(); });
  server->on("/api/library/shelves", HTTP_GET, [this] { handleGetLibraryShelves(); });
  server->on("/api/library/shelves/create", HTTP_POST, [this] { handlePostLibraryShelfCreate(); });
  server->on("/api/library/shelves/rename", HTTP_POST, [this] { handlePostLibraryShelfRename(); });
  server->on("/api/library/shelves/delete", HTTP_POST, [this] { handlePostLibraryShelfDelete(); });
  server->on("/api/library/assign", HTTP_POST, [this] { handlePostLibraryAssign(); });
  ...
  // Obsidian clipping-sync endpoints
...
void CrossPointWebServer::handleChicagoFont() const {
  server->sendHeader("Content-Encoding", "gzip");
  server->sendHeader("Cache-Control", "public, max-age=31536000, immutable");
  server->send_P(200, "font/ttf", ChicagoFlfTtf, ChicagoFlfTtfCompressedSize);
}
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (state.file) {
      ...
      if (state.error.isEmpty()) {
        ...
        RetroInkLibraryCatalog::noteChangedBook(filePath.c_str());
      }
    }
...
void CrossPointWebServer::handleRename() const {
  ...
  file.close();
  const bool success = BookMoveUtils::moveWithState(itemPath.c_str(), newPath.c_str());
...
void CrossPointWebServer::handleDelete() const {
  ...
  for (const auto& p : paths) {
    ...
    const bool folder = f && f.isDirectory();
    if (folder) {
      f.close();
    ...
    if (success) RetroInkLibraryCatalog::noteChangedBook(itemPath.c_str(), folder);
    if (!success) {
...
// --- Library shelf handlers ---
//
// Books are identified to the frontend by fingerprint (RetroInkLibraryCatalog
// ::Record::fingerprint), a content-based hash that survives renames/moves -
// the same identity the catalog already uses internally. It's serialized as
// a JSON string, not a number: it's a full 64-bit value and JavaScript's
// JSON numbers are IEEE-754 doubles, which silently lose precision above
// 2^53. A round-tripped string avoids that.
...
namespace {
// Drives a RetroInkLibraryCatalog scan to completion synchronously. Unlike
// RetroInkLibraryActivity (which steps once per frame to stay responsive),
// a web request handler has no per-frame budget to respect, so this just
// runs the scan to completion (or failure) before returning.
bool loadLibraryCatalog(RetroInkLibraryCatalog& catalog) {
  if (catalog.openCached()) return true;
  if (!catalog.beginScan()) return false;
  while (catalog.isScanning() && !catalog.failed()) {
    catalog.stepScan();
  }
...
void CrossPointWebServer::handleGetLibraryBooks() const {
  RetroInkLibraryCatalog catalog;
  if (!loadLibraryCatalog(catalog) ||
      !catalog.buildView(RetroInkLibraryCatalog::Shelf::All, RetroInkLibraryCatalog::Sort::Title)) {
    server->send(500, "text/plain", "Could not read the library catalog");
    return;
  }

  RetroInkCustomShelves shelves;
  shelves.load();  // A missing/empty store just means no custom assignments yet.
  ...
  constexpr size_t outputSize = sizeof(output);
  JsonDocument doc;
  RetroInkLibraryCatalog::Record record;
  const uint32_t count = catalog.viewCount();
  for (uint32_t i = 0; i < count; ++i) {
    if (!catalog.viewRecord(i, record)) continue;
    doc.clear();
    doc["path"] = record.path;
    doc["title"] = record.title;
    doc["author"] = record.author;
    doc["fingerprint"] = std::to_string(record.fingerprint);
    const bool activity = record.flags & RetroInkLibraryCatalog::FLAG_ACTIVITY;
    const bool finished = record.flags & RetroInkLibraryCatalog::FLAG_FINISHED;
    doc["toRead"] = !activity && !finished;
    doc["reading"] = activity && !finished;
    doc["finished"] = finished;
    doc["favorite"] = static_cast<bool>(record.flags & RetroInkLibraryCatalog::FLAG_FAVORITE);
    doc["shelfId"] = shelves.shelfForFingerprint(record.fingerprint);

    const size_t written = serializeJson(doc, output, outputSize);
    if (written >= outputSize) continue;  // Skip pathologically long metadata rather than truncate JSON.
...
void CrossPointWebServer::handleGetLibraryShelves() const {
  RetroInkCustomShelves shelves;
  shelves.load();
  ...
  server->send(200, "application/json", "");
  server->sendContent("[");
  const auto& list = shelves.shelves();
  for (size_t i = 0; i < list.size(); ++i) {
    doc.clear();
    doc["id"] = list[i].id;
    doc["name"] = list[i].name;
    doc["sortOrder"] = list[i].sortOrder;
    const size_t written = serializeJson(doc, output, outputSize);
    if (written >= outputSize) continue;
    if (i > 0) server->sendContent(",");
    server->sendContent(output);
...
void CrossPointWebServer::handlePostLibraryShelfCreate() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
  ...
  RetroInkCustomShelves shelves;
  shelves.load();
  const uint16_t id = shelves.createShelf(name);
  if (id == RetroInkCustomShelves::kNoShelf) {
    server->send(400, "text/plain", "Invalid shelf name, or shelf limit reached");
    return;
  }
  if (!shelves.save()) {
    server->send(500, "text/plain", "Could not save the new shelf");
    return;
  }
...
void CrossPointWebServer::handlePostLibraryShelfRename() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
  ...
  RetroInkCustomShelves shelves;
  shelves.load();
  if (!shelves.renameShelf(id, name)) {
    server->send(400, "text/plain", "Unknown shelf, or invalid name");
    return;
  }
  if (!shelves.save()) {
    server->send(500, "text/plain", "Could not save the rename");
    return;
  }
  server->send(200, "text/plain", "OK");
}

void CrossPointWebServer::handlePostLibraryShelfDelete() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
  ...
  RetroInkCustomShelves shelves;
  shelves.load();
  if (!shelves.deleteShelf(id)) {
    server->send(400, "text/plain", "Unknown shelf");
    return;
  }
  if (!shelves.save()) {
    server->send(500, "text/plain", "Could not save the deletion");
    return;
  }
  server->send(200, "text/plain", "OK");
}

void CrossPointWebServer::handlePostLibraryAssign() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
  ...
  const std::string fingerprintStr = doc["fingerprint"] | std::string("");
  if (fingerprintStr.empty()) {
    server->send(400, "text/plain", "Missing fingerprint");
    return;
  }
  const uint64_t fingerprint = strtoull(fingerprintStr.c_str(), nullptr, 10);
  const uint16_t shelfId = doc["shelfId"] | 0;

  RetroInkCustomShelves shelves;
  shelves.load();
  if (!shelves.assignToShelf(fingerprint, shelfId)) {
    server->send(400, "text/plain", "Unknown shelf");
    return;
  }
  if (!shelves.save()) {
    server->send(500, "text/plain", "Could not save the assignment");
    return;
  }
  server->send(200, "text/plain", "OK");
}
...
// ---- Obsidian Clipping Sync API ----

void CrossPointWebServer::handleGetObsidianConfig() const {
  const ObsidianSyncConfig& cfg = OBSIDIAN_STORE.getConfig();

  JsonDocument doc;
  doc["enabled"] = cfg.enabled;
  doc["mode"] = obsidianTargetModeToJson(cfg.mode);
  doc["baseUrl"] = cfg.baseUrl;
  doc["fallbackBaseUrl"] = cfg.fallbackBaseUrl;
  doc["notePathTemplate"] = cfg.notePathTemplate;
  doc["insecureTls"] = cfg.insecureTls;
  doc["autoSyncOnFileTransfer"] = cfg.autoSyncOnFileTransfer;
  // Never expose the API key over the API. Only indicate whether one is set.
  doc["hasApiKey"] = !cfg.apiKey.empty();
  doc["pendingCount"] = static_cast<uint32_t>(ObsidianPendingQueue::count());
...
void CrossPointWebServer::handlePostObsidianConfig() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
  ...
  ObsidianSyncConfig cfg = OBSIDIAN_STORE.getConfig();
  cfg.enabled = doc["enabled"] | cfg.enabled;
  cfg.mode = obsidianTargetModeFromJson(doc["mode"] | obsidianTargetModeToJson(cfg.mode));
  cfg.baseUrl = doc["baseUrl"] | cfg.baseUrl;
  cfg.fallbackBaseUrl = doc["fallbackBaseUrl"] | cfg.fallbackBaseUrl;
  cfg.notePathTemplate = doc["notePathTemplate"] | cfg.notePathTemplate;
  cfg.insecureTls = doc["insecureTls"] | cfg.insecureTls;
  cfg.autoSyncOnFileTransfer = doc["autoSyncOnFileTransfer"] | cfg.autoSyncOnFileTransfer;

  // Only overwrite the stored key when the caller actually sent one. The web
  // UI omits it once set so re-saving other fields doesn't clear it.
  const bool hasApiKeyField = doc["apiKey"].is<const char*>() || doc["apiKey"].is<std::string>();
  if (hasApiKeyField) {
    cfg.apiKey = doc["apiKey"] | std::string("");
  }

  if (!OBSIDIAN_STORE.setConfig(cfg)) {
    server->send(500, "text/plain", "Failed to save Obsidian sync config");
    return;
  }
...
void CrossPointWebServer::handlePostObsidianSync() const {
  // Wi-Fi is already up (this handler only runs while the web server is),
