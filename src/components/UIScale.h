...
inline UIScaleSpec uiScaleSpec() {
  UIScaleSpec spec{};
  // RetroInk's pixel font is designed for the large metrics. Treat the theme
  // as large even if an older settings file or remote client supplies Small.
  const uint8_t scale =
      SETTINGS.uiTheme == CrossPointSettings::UI_THEME::SYSTEM6 ? CrossPointSettings::UI_SCALE_LARGE : SETTINGS.uiScale;
  switch (scale) {
