...
inline freeink::ui::BitmapRef listIconFor(const UIIcon icon, const int size = 24) {
  if (size >= 32) {
    switch (icon) {
      ...
      case UIIcon::BookmarkIcon:
        return freeink::ui::bitmapFromIcon(icon_book_marked_32);
      case UIIcon::File:
      ...
      case UIIcon::Obsidian:
        return freeink::ui::bitmapFromIcon(icon_obsidian_32);
      default:
        return {};
    }
  }
  switch (icon) {
    ...
    case UIIcon::BookmarkIcon:
      return freeink::ui::bitmapFromIcon(icon_book_marked_24);
    case UIIcon::File:
    ...
    case UIIcon::Obsidian:
      return freeink::ui::bitmapFromIcon(icon_obsidian_24);
    default:
      return {};
  }
}
...
inline void drawUiEmptyState(freeink::ui::Screen<MaxInteractions>& screen, const char* message,
                             const UIIcon icon = UIIcon::File) {
  ...
  const fui::BitmapRef bitmap = listIconFor(icon, 32);
  fui::TextStyle text = screen.theme().bodyText;
  text.align = fui::TextAlign::Center;
  const int16_t lineHeight = screen.target().lineHeight(text.font);
  const int16_t blockHeight = static_cast<int16_t>(32 + screen.theme().spaceLg + lineHeight);
  const int16_t top = static_cast<int16_t>(body.y + std::max<int16_t>(0, (body.height - blockHeight) / 2));
  if (bitmap) {
    screen.target().bitmap(fui::Rect{static_cast<int16_t>(body.x + (body.width - 32) / 2), top, 32, 32}, bitmap,
                           fui::BitmapMode::Center, fui::Paint::solid(fui::Color::Black));
  }
  screen.target().text(
      fui::Rect{body.x, static_cast<int16_t>(top + 32 + screen.theme().spaceLg), body.width, lineHeight}, message, text);
}
