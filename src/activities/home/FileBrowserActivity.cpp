...
void FileBrowserActivity::promptDeleteFile(const std::string& fullPath, const std::string& entry) {
  auto handler = [this, fullPath](const ActivityResult& res) {
    ...
    RetroInkLibraryCatalog::noteChangedBook(fullPath.c_str());
...
void FileBrowserActivity::promptDeleteDirectory(const std::string& fullPath, const std::string& entry,
  ...
  auto handler = [this, dirPath](const ActivityResult& res) {
    ...
    RetroInkLibraryCatalog::noteChangedBook(dirPath.c_str(), true);
...
void FileBrowserActivity::loop() {
  ...
  const int pathFontId = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 ? UI_10_FONT_ID : SMALL_FONT_ID;
  const int contentBottom = renderer.getScreenHeight() - metrics.buttonHintsHeight -
                            renderer.getLineHeight(pathFontId) - metrics.verticalSpacing;
...
void FileBrowserActivity::buildListScreen(UiApp::ScreenType& screen) {
  ...
  // deepest directory stays visible.
  {
    ...
    renderer.drawText(pathFontId, band.x + metrics.contentSidePadding, pathY, pathDisplay);
  }
  ...
  if (totalEntries == 0) {
    ...
    drawUiEmptyState(screen, emptyMessage, UIIcon::Folder);
    return;
  }
...
void FileBrowserActivity::render(RenderLock&&) {
  ...
  if (SETTINGS.uiTheme != CrossPointSettings::SYSTEM6 && !mappedInput.hasTouch() && mode == Mode::Books &&
      basepath == "/") {
