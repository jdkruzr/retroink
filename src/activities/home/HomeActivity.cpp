...
int HomeActivity::getMenuItemCount() const {
  const auto& metrics = UITheme::getInstance().getMetrics();
  int count = buildHomeMenuItems(hasOpdsServers, hasReadingStats, hasBookmarks, hasClippings).size();
      ...
      auto activateMinimalMenuAction = [this, &menuItems]() {
        switch (menuItems[minimalMenuIndex].action) {
          case HomeMenuAction::BrowseFiles:
            onFileBrowserOpen();
            break;
          case HomeMenuAction::RetroLibrary:
            break;
  ...
  auto activateHomeMenuAction = [this](const HomeMenuAction action) {
    switch (action) {
      case HomeMenuAction::BrowseFiles:
        ...
        break;
      case HomeMenuAction::RetroLibrary:
        startActivityForResult(std::make_unique<RetroInkLibraryActivity>(renderer, mappedInput),
      ...
      case HomeMenuAction::Bookmarks:
        if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6) {
          startActivityForResult(std::make_unique<SavedItemsHomeActivity>(renderer, mappedInput, true),
                                 [this](const ActivityResult&) { requestUpdate(); });
        } else {
          onSavedItemsOpen();
        }
