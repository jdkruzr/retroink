...
void SavedItemsHomeActivity::reloadSavedBooks() {
  ...
  if (!clippingsOnly) BookmarkStore::getAllBookmarkedBooks(bookmarkedBooks);
...
void SavedItemsHomeActivity::buildListScreen(UiApp::ScreenType& screen) {
  ...
  if (books.empty()) {
    drawUiEmptyState(screen, clippingsOnly ? tr(STR_NO_CLIPPINGS) : tr(STR_NO_BOOKMARKS), UIIcon::BookmarkIcon);
    return;
  }
...
void SavedItemsHomeActivity::render(RenderLock&&) {
  ...
  if (mappedInput.hasTouchHardware()) {
    TouchHeaderBackButton::drawCompact(renderer,
                                       clippingsOnly ? tr(STR_CLIPPINGS) : tr(STR_BOOKMARKS_AND_CLIPPINGS));
  } else {
    CompactHeader::drawTitle(renderer, clippingsOnly ? tr(STR_CLIPPINGS) : tr(STR_BOOKMARKS_AND_CLIPPINGS));
  }
  uiReady = false;
  app.render();
  uiReady = true;
