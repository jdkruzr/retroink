...
bool toggleBookCompleted(const std::string& fullPath, const std::string& displayName, bool& completed) {
  ...
  globalStats.save();
  RetroInkLibraryCatalog::syncBookState(fullPath.c_str());
  ...
  if ((isEpub || isXtc) && completed && SETTINGS.moveFinishedToReadFolder && fullPath.rfind("/Read/", 0) != 0) {
    const std::string dstPath = BookMoveUtils::buildReadFolderDestination(fullPath);
    LOG_INF("BookActions", "Moving completed book: %s -> %s", fullPath.c_str(), dstPath.c_str());
    if (!BookMoveUtils::moveWithState(fullPath, dstPath, !SETTINGS.removeReadBooksFromRecents)) {
