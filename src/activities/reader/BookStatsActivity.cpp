...
void BookStatsActivity::onEnter() {
  ...
  if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 && returnToHomeOnExit) page = Page::ThisDevice;
  requestUpdate();
}
...
bool BookStatsActivity::showNextStatsPage() {
  if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 && page != Page::EditDates) {
    page = page == Page::PerBook ? Page::ThisDevice : page == Page::ThisDevice ? Page::AllDevices : Page::PerBook;
    requestUpdate();
    return true;
  }
...
bool BookStatsActivity::showPreviousStatsPage() {
  if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 && page != Page::EditDates) {
    page = page == Page::PerBook ? Page::AllDevices : page == Page::AllDevices ? Page::ThisDevice : Page::PerBook;
    requestUpdate();
    return true;
  }
...
void BookStatsActivity::loop() {
  ...
  if (usesNoRtcSingleScreenLayout() && SETTINGS.uiTheme != CrossPointSettings::SYSTEM6) {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Back)) {
    if (SETTINGS.uiTheme != CrossPointSettings::SYSTEM6) mappedInput.suppressNextBackRelease();
    exitStatsActivity();
    return;
  }

  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    if (SETTINGS.uiTheme != CrossPointSettings::SYSTEM6) mappedInput.suppressNextConfirmRelease();
    if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6) {
      startActivityForResult(
          std::make_unique<RetroInkFocusDeskActivity>(
              renderer, mappedInput, RetroInkFocusDeskActivity::Entry::Stats,
              returnToHomeOnExit ? std::string{} : APP_STATE.openEpubPath),
  ...
  if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6) {
    if (upOrLeftPressed) showPreviousStatsPage();
    if (downOrRightPressed) showNextStatsPage();
    return;
  }
...
void BookStatsActivity::render(RenderLock&&) {
  ...
  if (retroInk && page != Page::EditDates) {
    switch (page) {
      case Page::PerBook:
        RetroInkReadingDeskView::renderBookStatus(renderer, &mappedInput, bookTitle, stats, progressPercent,
                                                   estimatedTimeLeftSeconds);
        break;
      case Page::ThisDevice:
        RetroInkReadingDeskView::renderToday(renderer, &mappedInput, globalStats);
        break;
      case Page::AllDevices:
        RetroInkReadingDeskView::renderYear(renderer, &mappedInput);
        break;
      case Page::EditDates:
        break;
    }
