...
inline int getTopClockStatusBarHeight() {
  const bool goalBadge = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 &&
                         SETTINGS.showReadingGoalCountdown && SETTINGS.shouldTrackReadingStats() &&
                         halClock.isAvailable();
  if (!shouldShowTopClockStatusBar() && !goalBadge) {
    return 0;
  }
