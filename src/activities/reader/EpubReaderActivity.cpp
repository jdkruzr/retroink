...
void moveFinishedBookToReadFolder(const std::string& srcPath, const std::string& dstPath,
  ...
  (void)oldCachePath;
  (void)author;
  if (!BookMoveUtils::moveWithState(srcPath, dstPath, !SETTINGS.removeReadBooksFromRecents)) {
...
void EpubReaderActivity::onExit() {
  ...
  if (epub) RetroInkLibraryCatalog::syncBookState(epub->getPath().c_str());
...
void EpubReaderActivity::loop() {
  ...
  if (pageShownAtMs && !RenderLock::peek() && !activeFootnotePreview && goalCountdown.shouldPoll(millis())) {
    ...
    if (goalCountdown.needsUpdate(goal)) {
      RenderLock lock(*this);
      goalCountdown.tick(renderer, goal, ReaderUtils::readerDarkModeEnabled());
    }
  }
...
void EpubReaderActivity::onReaderMenuConfirm(EpubReaderMenuActivity::MenuAction action) {
  switch (action) {
    ...
    case EpubReaderMenuActivity::MenuAction::FOCUS_SESSION: {
      pauseReadingPaceTimer("focus_session");
      startActivityForResult(
          std::make_unique<FocusSessionActivity>(renderer, mappedInput, SETTINGS.focusDurationMinutes(), epub->getPath()),
          [this](const ActivityResult&) { resumeReadingPaceTimer("focus_session_return"); });
      break;
    }
    ...
    if (!result.isCancelled) {
      ...
      if (!clip.text.empty()) {
        ...
        if (addResult == ClippingStore::AddResult::Added) {
          ...
          if (exported && OBSIDIAN_STORE.getConfig().enabled) {
            // Local-only append (no network): queued for the next Obsidian
            // sync, on-demand or auto-triggered when File Transfer starts.
            ObsidianPendingClipping pending;
            pending.book = bookTitle;
            pending.author = author;
            pending.chapter = chapterTitle;
            pending.page = static_cast<int>(clip.sectionPage) + 1;
            pending.text = clip.text;
            pending.timestamp = static_cast<uint32_t>(time(nullptr));
            ObsidianPendingQueue::append(pending);
          }
        }
