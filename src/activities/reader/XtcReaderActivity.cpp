...
void XtcReaderActivity::onExit() {
  ...
  commitReadingStats();
  if (xtc) RetroInkLibraryCatalog::syncBookState(xtc->getPath().c_str());
  ...
  if (xtc && stats.isCompleted && SETTINGS.moveFinishedToReadFolder && xtc->getPath().rfind("/Read/", 0) != 0) {
    const std::string oldPath = xtc->getPath();
    const std::string newPath = BookMoveUtils::buildReadFolderDestination(oldPath);
    xtc.reset();  // release SD handles before moving this file and its cache
    if (!BookMoveUtils::moveWithState(oldPath, newPath, !SETTINGS.removeReadBooksFromRecents))
      LOG_ERR("XTR", "Could not move finished XTC; journal will retry after reboot");
  } else {
    xtc.reset();
  }
}
...
void XtcReaderActivity::loop() {
  ...
  if (pageShownAtMs && !RenderLock::peek() && currentPage < xtc->getPageCount() &&
      goalCountdown.shouldPoll(millis())) {
    uint32_t pageSeconds = 0;
    currentPageReadingSecondsForStats(pageSeconds, "goal_countdown");
    const auto goal = RetroInkGoalCountdown::sample(sessionStartLocalDateTime, sessionReadingSeconds, pageSeconds);
    if (goalCountdown.needsUpdate(goal)) {
      RenderLock lock(*this);
      goalCountdown.tick(renderer, goal, false);
    }
  }
...
void XtcReaderActivity::commitReadingStats() {
  ...
  if (elapsedSecs >= 10) {
    ...
    if (hasSessionStartLocalDateTime) {
      ...
      ReadingDeskStore::recordReadingSpan(sessionStartLocalDateTime, elapsedSecs);
    }
...
void XtcReaderActivity::onReaderMenuConfirm(const int action) {
  switch (static_cast<XtcReaderMenuActivity::MenuAction>(action)) {
    ...
    case XtcReaderMenuActivity::MenuAction::READING_STATS:
      openReadingStats();
      break;
    case XtcReaderMenuActivity::MenuAction::FOCUS_SESSION:
      pauseReadingStatsTimer("focus_session");
      startActivityForResult(
          std::make_unique<FocusSessionActivity>(renderer, mappedInput, SETTINGS.focusDurationMinutes(), xtc->getPath()),
          [this](const ActivityResult&) { resumeReadingStatsTimer("focus_session_return"); });
      break;
...
void XtcReaderActivity::renderPage() {
  ...
  if (bitDepth == 2) {
    ...
    const auto goal = RetroInkGoalCountdown::sample(sessionStartLocalDateTime, sessionReadingSeconds, 0);
    if (goal.valid) {
      goalCountdown.drawOnPage(renderer, goal, false);
      int top, right, bottom, left;
      renderer.getOrientedViewableTRBL(&top, &right, &bottom, &left);
      (void)bottom;
      (void)left;
      const int badgeW = std::min(112, renderer.getScreenWidth() / 4);
      renderer.displayWindow(renderer.getScreenWidth() - right - badgeW - 4,
                             top + UITheme::getInstance().getMetrics().topPadding, badgeW, 32);
    }
    return;
  }
  ...
  uint32_t pageSeconds = 0;
  currentPageReadingSecondsForStats(pageSeconds, "goal_countdown_render");
  goalCountdown.drawOnPage(renderer,
                           RetroInkGoalCountdown::sample(sessionStartLocalDateTime, sessionReadingSeconds, pageSeconds),
                           false);
