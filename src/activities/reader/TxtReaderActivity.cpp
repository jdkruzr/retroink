...
void TxtReaderActivity::onEnter() {
  Activity::onEnter();
  getCurrentLocalReadingStatsDateTime(sessionStartLocalDateTime);
...
void TxtReaderActivity::onExit() {
  ...
  if (SETTINGS.shouldTrackReadingStats() && sessionStartLocalDateTime.isValid() && sessionReadingSeconds >= 10) {
    ReadingDeskStore::recordReadingSpan(sessionStartLocalDateTime, sessionReadingSeconds);
  }
  Activity::onExit();
  ...
  APP_STATE.saveToFile();
  if (txt) RetroInkLibraryCatalog::syncBookState(txt->getPath().c_str());
  txt.reset();
}
...
void TxtReaderActivity::loop() {
  if (txt && pageShownAtMs && !RenderLock::peek() && goalCountdown.shouldPoll(millis())) {
    const auto goal = RetroInkGoalCountdown::sample(sessionStartLocalDateTime, sessionReadingSeconds,
                                                     currentPageReadingSeconds());
...
uint32_t TxtReaderActivity::currentPageReadingSeconds() const {
  if (!SETTINGS.shouldTrackReadingStats() || !pageShownAtMs) return 0;
  const uint32_t seconds = static_cast<uint32_t>((millis() - pageShownAtMs) / 1000UL);
  return seconds <= SETTINGS.getReadingIdleTimeThresholdSeconds() ? seconds : 0;
}

void TxtReaderActivity::recordCurrentPageReadingTime() {
  const uint32_t seconds = currentPageReadingSeconds();
