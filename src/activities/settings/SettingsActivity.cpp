...
void drawCenteredTextLine(const GfxRenderer& renderer, const int font, const int pageWidth, const int y,
                          const std::string& text) {
  const int labelWidth = renderer.getTextWidth(font, text.c_str());
  const int labelX = (pageWidth - labelWidth) / 2;
  renderer.drawText(font, labelX, y, text.c_str());
}
...
void drawSystemVersionFooter(const GfxRenderer& renderer, const int pageWidth, const int pageHeight,
  ...
  std::string label = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 ? tr(STR_SYSTEM6_DESKTOP) : tr(STR_CROSSINK);
  label += " " CROSSINK_VERSION;
  const int font = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 ? UI_10_FONT_ID : SMALL_FONT_ID;
  const int maxWidth = pageWidth - systemVersionFooterSideMargin * 2;
  const int lineHeight = renderer.getLineHeight(font);
  const int bottomLineY = pageHeight - metrics.buttonHintsHeight - systemVersionFooterBottomInset - lineHeight;

  if (renderer.getTextWidth(font, label.c_str()) <= maxWidth) {
    drawCenteredTextLine(renderer, font, pageWidth, bottomLineY, label);
    return;
  }
  ...
  for (size_t i = 1; i < label.size(); i++) {
    ...
    if (renderer.getTextWidth(font, firstLine.c_str()) > maxWidth) break;

    fallbackBreak = i;
    const std::string secondLine = label.substr(i);
    if (renderer.getTextWidth(font, secondLine.c_str()) <= maxWidth) {
      preferredBreak = i;
    }
  }
  ...
  const std::string firstLine =
      lineBreak == std::string::npos ? renderer.truncatedText(font, label.c_str(), maxWidth) : label.substr(0, lineBreak);
  const std::string secondLine = lineBreak == std::string::npos
                                     ? ""
                                     : renderer.truncatedText(font, label.substr(lineBreak).c_str(), maxWidth);
  drawCenteredTextLine(renderer, font, pageWidth, bottomLineY - lineHeight, firstLine);
  drawCenteredTextLine(renderer, font, pageWidth, bottomLineY, secondLine);
}
...
void SettingsActivity::toggleCurrentSetting() {
  ...
  if (setting.valuePtr == &CrossPointSettings::uiScale && SETTINGS.uiTheme == CrossPointSettings::UI_THEME::SYSTEM6) {
...
std::string SettingsActivity::settingValueText(const SettingInfo& setting) {
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    if (SETTINGS.uiTheme == CrossPointSettings::SYSTEM6) {
      return SETTINGS.*(setting.valuePtr) ? "[X]" : "[ ]";
    }
...
void SettingsActivity::buildSettingsScreen(UiApp::ScreenType& screen) {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int versionFooterFont = SETTINGS.uiTheme == CrossPointSettings::SYSTEM6 ? UI_10_FONT_ID : SMALL_FONT_ID;
  const int versionFooterReserve =
      selectedCategoryIndex == 3 ? renderer.getLineHeight(versionFooterFont) * 2 + systemVersionFooterBottomInset : 0;
  ...
  screen.setContentMargin(fui::Insets{static_cast<int16_t>(settingsTabBarTop(metrics)), 0,
                                      static_cast<int16_t>(metrics.buttonHintsHeight + versionFooterReserve), 0});
  ...
  const bool system6Tabs = SETTINGS.uiTheme == CrossPointSettings::UI_THEME::SYSTEM6;
  ...
  } else if (borderedTabs && system6Tabs) {
    // RetroInk uses the classic Mac selected-menu treatment. Keep the current
    // category visibly inverted while focus moves through its settings rows.
    tabStyles.explicitlySet = true;
    tabStyles.normal.foreground = fui::Paint::solid(fui::Color::Black);
    tabStyles.selected.background = fui::Paint::solid(fui::Color::Black);
    tabStyles.selected.foreground = fui::Paint::solid(fui::Color::White);
    tabStyles.selected.radius = 0;
    tabStyles.focused = tabStyles.selected;
    tabStyles.active = tabStyles.selected;
    tabProps.tabStyles = tabStyles;
